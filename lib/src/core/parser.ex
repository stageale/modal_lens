defmodule Src.Core.Parser do
    ...
    def parse_nitpick_text(text, opts \\ []) do
        ...
        {initial_world, warnings} = case parse_initial_world(text) do
        ...
        requested_atoms = if auto_atoms do
        ...
        {valuations, final_warnings} = Enum.reduce(requested_atoms, {%{}, warnings}, fn atom, {acc_vals, acc_warns} ->
    ...
    defp parse_kind_and_cardinality(text) do
        cond do
            ...
            String.contains?(text, "Nitpick found no counterexample") ->
                raise ArgumentError, "Nitpick found no counterexample; no finite model/countermodel to parse."

            true ->
                raise ArgumentError, "Could not detect 'Nitpick found a model/countermodel for card i = n'."
        end
    end
    #*
    ...
    defp parse_unary_predicate(text, atom, cardinality) do
        case extract_assignment_block(text, atom, true) do
            nil -> nil
            assignment ->
                found = for [_, w, val] <- Regex.scan(bool_assign_regex(), assignment),
                            idx = String.to_integer(w) - 1,
                            into: %{}, do: {idx, val == "True"}
