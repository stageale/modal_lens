...
defmodule Src.Interface.RefinementCLITest do
  ...
  test "verbalizes the completed report with the requested Ollama model", context do
    ...
    output = capture_io(fn ->
      assert CLI.main(arguments(context) ++ [
        "--verbalization-backend", "ollama", "--verbalization-model", "qwen-fixture"
      ]) == 0
    end)
  ...
  defp arguments(context) do
    ["refine", context.theory, "-o", context.run.output_dir, "--max-models", "1",
      "--max-refinement-rounds", "1", "--no-render-graph", "--atoms", "p,q"]
  end
